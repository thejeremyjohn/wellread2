...
import 'package:wellread2frontend/providers/book_notifier.dart';
import 'package:wellread2frontend/providers/books_notifier.dart';
import 'package:wellread2frontend/providers/bookshelves_notifier.dart';
...
class _BookPageState extends ConsumerState<BookPage> {
  ...
  Widget build(BuildContext context) {
    final bookNotifier = ref.watch(bookNotifierProvider(widget.bookId));
    ...
    // //   ],
    // // };
    // // final BooksGetArgs parameters = (
    // //   {'id': widget.bookId},
    // //   ['avg_rating', 'my_rating', 'my_shelves', 'n_reviews', 'n_ratings'],
    // // );
    ...
    //   ],
    // );
    // final booksNotifier = ref.watch(
    //   // booksNotifierProvider((
    //   //   {'id': widget.bookId},
    //   //   ['avg_rating', 'my_rating', 'my_shelves', 'n_reviews', 'n_ratings'],
    //   // )),
    //   booksNotifierProvider(parameters),
    //   // booksNotifierProvider(widget.bookId),
    // );

    final bookshelvesNotifier = ref.watch(
      bookshelvesNotifierProvider(_myUserId.toString()),
    );

    Widget shelfButtonDialogs = bookshelvesNotifier.when(
      data: (bookshelves) {
        ...
        // String? myShelfName = bookNotifier.asData?.value.myShelf!.name;
        String? myShelfName = bookNotifier
            .whenData((b) => b.myShelf!.name)
            .value;
        ...
        return SizedBox(
          ...
          child: ElevatedButton(
              ...
              builder: (BuildContext context) {
                return Consumer(
                  builder: (context, ref, _) {
                    final bookNotifier = ref.watch(
                      bookNotifierProvider(widget.bookId),
                    );
                    String? myShelfName = bookNotifier
                        .whenData((b) => b.myShelf!.name)
                        .value;
                    ...
                    return bookshelvesNotifier.when(
                      data: (bookshelves) {
                        Iterable<Bookshelf> shelves = bookshelves.take(3);
                        List<Bookshelf> tags = bookshelves.skip(3).toList();
                              ...
                              return SizedBox(
                                ...
                                child: ElevatedButton.icon(
                                  onPressed: () async {
                                    ...
                                    print('changeEssentialShelf would call');
                                    Book book = bookNotifier
                                        .whenData((b) => b)
                                        .value!;
                                    if (book.myShelf == shelf) return;
                                    await book.myShelf!.removeBook(
                                      book,
                                      deleteTags: false,
                                    );
                                                ...
                                                children: <Widget>[
                                                  ...
                                                  ElevatedButton(
                                                    onPressed: () {
                                                      ...
                                                      print(
                                                        'removeFromEssentialShelf would call',
                                                      );
                                                      context.pop();
                                                    },
            ...
            child: Text(myShelfName ?? 'unshelved'),
          ),
        );
      },
      error: (error, stackTrace) => Text(error.toString()),
    ...
    // Widget coverAndShelf = booksNotifier.when(
    //   data: (books) {
    //     Book book = books.first;
    Widget coverAndShelf = bookNotifier.when(
      data: (book) {
        return Column(
          children: [
            book.cover,
            ...
            RatingBar.builder(
              ...
              onRatingUpdate: (rating) {
                if (rating != book.myRating) {
                  print('reviewCreateOrUpdate would call');
    ...
    // Widget bookDetails = booksNotifier.when(
    //   data: (books) {
    //     Book book = books.first;
    Widget bookDetails = bookNotifier.when(
      data: (book) {
        return Column(
